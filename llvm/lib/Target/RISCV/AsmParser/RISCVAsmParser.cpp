...
void RISCVAsmParser::emitCapLoadLocalCap(MCInst &Inst, SMLoc IDLoc,
  ...
  emitAuipccInstPair(DestReg, DestReg, Symbol, RISCVMCExpr::VK_RISCV_PCREL_HI,
                     HasRVY ? RISCV::CADDI : RISCV::CIncOffsetImm, IDLoc, Out);
}
...
void RISCVAsmParser::emitCapLoadGlobalCap(MCInst &Inst, SMLoc IDLoc,
  ...
  unsigned SecondOpcode =
      HasRVY ? RISCV::CLC : (isRV64() ? RISCV::CLC_128 : RISCV::CLC_64);
...
void RISCVAsmParser::emitCapLoadTLSGDCap(MCInst &Inst, SMLoc IDLoc,
  ...
  const MCExpr *Symbol = Inst.getOperand(1).getExpr();
  const bool HasRVY = STI->hasFeature(RISCV::FeatureStdExtY);
  const unsigned IncOpc = HasRVY ? RISCV::CADDI : RISCV::CIncOffsetImm;

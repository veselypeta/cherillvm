# RUN: llvm-mc %s -triple=riscv32 -mattr=+h,+y,+cap-mode -riscv-no-aliases -show-encoding \
# RUN:     | FileCheck -check-prefixes=CHECK,CHECK-INST %s
# RUN: llvm-mc %s -triple=riscv64 -mattr=+h,+y,+cap-mode -riscv-no-aliases -show-encoding \
# RUN:     | FileCheck -check-prefixes=CHECK,CHECK-INST %s
# RUN: llvm-mc -filetype=obj -triple riscv32 -mattr=+h,+y,+cap-mode < %s \
# RUN:     | llvm-objdump --mattr=+h,+y,+cap-mode -M no-aliases -d - \
# RUN:     | FileCheck -check-prefix=CHECK-INST %s
# RUN: llvm-mc -filetype=obj -triple riscv64 -mattr=+h,+y,+cap-mode < %s \
# RUN:     | llvm-objdump --mattr=+h,+y,+cap-mode -M no-aliases -d - \
# RUN:     | FileCheck -check-prefix=CHECK-INST %s

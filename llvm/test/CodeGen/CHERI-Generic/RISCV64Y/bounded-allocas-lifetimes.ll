...
; RUN: llc -mtriple=riscv64 --relocation-model=pic -target-abi l64pc128d -mattr=+y,+cap-mode,+f,+d %s -o - --stop-after=finalize-isel | FileCheck %s
...
define void @dynamic_alloca(i64 zeroext %n) {
  ...
  ; CHECK-NEXT:   [[YRAM:%[0-9]+]]:gpr = YRAM [[ANDI]]
  ; CHECK-NEXT:   [[XORI:%[0-9]+]]:gpr = XORI [[YRAM]], -1
  ; CHECK-NEXT:   [[ADD:%[0-9]+]]:gpr = ADD [[ANDI]], killed [[XORI]]
  ; CHECK-NEXT:   [[AND:%[0-9]+]]:gpr = AND killed [[ADD]], [[YRAM]]
  ; CHECK-NEXT:   ADJCALLSTACKDOWNCAP 0, 0, implicit-def dead $c2, implicit $c2
  ...
  ; CHECK-NEXT:   [[YRAM1:%[0-9]+]]:gpr = YRAM [[ANDI]]
  ; CHECK-NEXT:   [[AND1:%[0-9]+]]:gpr = AND killed [[SUB]], killed [[YRAM1]]
